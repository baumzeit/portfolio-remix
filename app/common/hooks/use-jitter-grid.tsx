...
export const useJitterGrid = ({
  ...
  const gridData = useMemo(() => {
    try {
      if (width && minHeight) {
        ...
        const height =
          minTilePixels && pixelsPerItem < minTilePixels
            ? (minItems * minTilePixels) / width
            : minHeight;
        ...
        const virtualColumns = aspect
          ? defaultSize * aspectFactor.x
          : defaultSize;
        ...
        const refCols =
          safeColumns * safeColumns - minItems >= safeColumns
            ? safeColumns - 1
            : safeColumns;
        const refRows =
          safeRows - Math.floor((safeRows * refCols - minItems) / refCols);
        ...
        const getNoise = (unitSize: number, jitter: number) =>
          unitSize * randomNormal(0, jitter)();

        const getColSpecs = (items: number): AxisSpecs => {
          const colWidth =
            (width - (marginLeft + marginRight)) / Math.max(items - 1, 1);
          const colDiff = Math.max(refCols - items, 0);
          const offset = colDiff ? (0.5 * (colWidth * colDiff)) / refCols : 0;
          const adjustedColWidth = colDiff
            ? (colWidth * items) / refCols
            : colWidth;
          return {
            positions: Array.from(
              { length: items },
              (_, idx) => marginLeft + offset + adjustedColWidth * idx
            ),
            interval: colWidth,
          };
        };
        const getRowSpecs = (rows: number): AxisSpecs => {
          const rowHeight =
            (height - (marginTop + marginBottom)) / Math.max(rows - 1, 1);
          return {
            positions: Array.from(
              { length: rows },
              (_, idx) => marginTop + rowHeight * idx
            ),
            interval: rowHeight,
          };
        };
        ...
        const getRowJitter = (idx: number) =>
          height * (idx % 2) ? height * 0.02 : height * -0.02;
        ...
        const actualRows =
          numOrphans > 0 && adoptedOrphans === numOrphans
            ? refRows - 1
            : refRows;
  ...
  return {
    getGridCoordinates: gridData ? getGridCoordinates(gridData) : undefined,
    gridSpecs: gridData,
  };
};
