...
export const restore = (svgNode: SVGSVGElement) => () => {
  ...
  if (!svg.classed("expose-view")) {
    svg
      .selectAll<SVGPathElement, EnrichedDatum>(`.cell`)
      .classed("hover-selected exposed", false);
  }
};
...
export const hoverCell = (svgNode: SVGSVGElement) => (hoveredId: string) => {
  ...
  svg
    .selectAll<SVGPathElement, EnrichedDatum>(`.cell`)
    .classed("hover-selected", (d) => d.id === hoveredId);
};

export const highlightCellsByAreaId =
  (svgNode: SVGSVGElement) => (highlightId: string | null) => {
    const svg = d3.select(svgNode);
    svg.selectAll<SVGGElement, EnrichedDatum>(".cell").each(function (d) {
      ...
      const shouldHighlight = Boolean(
        d.areas.find((area) => highlightId && area.id === highlightId)
      );

      if (highlightId && shouldHighlight) {
        highlightPath.style(
          "fill",
          `url(#diagonalHatchHighlight-${highlightId})`
        );
      }
