...
export const RenderSingularCell = ({
  width,
  datum,
  imageSize,
}: RenderSingularCellProps) => {
      ...
      if (cellRect) {
        ...
        const ratio =
          cellRect?.width && cellRect?.height
            ? cellRect.width / cellRect.height
            : 1;
        const relOffsetX = cellRect
          ? 0.5 - (d.x - cellRect.x) / cellRect.width
          : 0;
        const relOffsetY = cellRect
          ? 0.5 - (d.y - cellRect.y) / cellRect.height
          : 0;
        ...
        svg
          ...
          .style(
            "transform",
            `translate(${(exposeCellWidth * relOffsetX) / 2}px, ${
              (exposeCellHeight * relOffsetY) / 2
            }px)`
          );
      }
    }
...
function getIsolatedCellSvg(id?: string) {
  ...
  if (!originalSvg.empty()) {
    ...
    svg
      .attr("class", "voronoi expose-view")
      .attr("width", "100%")
      .attr("height", "30vh");
